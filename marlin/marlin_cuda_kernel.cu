...
__device__ inline FragB dequant(int q) {
  ...
  int lo = lop3<(0xf0 & 0xcc) | 0xaa>(q, LO, EX); //  0xf0 & 0xcc) | 0xaa 的意思是说对后边的三个参数执行 先 & 后 | 操作
...
b_sh_rd_delta=%d b_sh_stage=%d b_sh_wr_iters=%d s_gl_stride=%d s_sh_stride=%d s_sh_stage=%d s_gl_rd_delta=%d \n",
  ...
  if (print_enable && blockIdx.x == user_specified_blockidx && blockIdx.y == 0 && blockIdx.z == 0 && \
...
| slice_col/row=%d/%d",
  ...
  for (int i = 0; i < b_sh_wr_iters/*2*/; i++) {
    ...
    for (int j = 0; j < thread_m_blocks/*4*/; j++){
      ...
      int input = a_sh_rd_delta_o/*4*/ * i + a_sh_rd_delta_i/*128*/ * j + a_sh_rd/* by threadIdx.x */;
      int output = transform_a(input);
    ...
    if (pred) {
      ...
      for (int i = 0; i < a_sh_wr_iters/*2*/; i++) {
        cp_async4_pred( // copy 16 bytes.
          &sh_a_stage[a_sh_wr_trans[i]], /* by threadIdx.x */
      ...
      if (group_blocks/*8*/ != -1 && pipe % (group_blocks/*8*/ / thread_k_blocks/*4*/) == 0) {
        // when pipe=0 or pipe=2, meaning 8 k dimension blocks passed, 8*16=128 number (quantized by one scale)
  ...
  auto global_reduce = [&] (bool first = false, bool last = false) {
    ...
    if (threadIdx.x < active_threads/*128*/) {
      ...
      int c_gl_wr_delta_i/*16*/ = 4 * (active_threads/*128*/ / 32);
            // 这个4* 可以写成  4*8/8 意思是, 4个线程, 每个线程拿一个fp16(两个里只取一个), 再转成int4.
            // 另一种解释,
            // 见 tensor core layout的Csubtile, 这里的4 是 当前线程把一行8个Csubtile里的两个数的第一个数拿出来合成一个int4
            // 一个warp 一行4个线程, 就写4个int4, C tile一行一共有4个warp, 因此写 4*4个int4
      ...
      if (!first) {
        ...
        // 这部分后执行, 因为要等待一个slice底下的先reduce完.
        #pragma unroll
        for (int i = 0; i < thread_m_blocks * 4; i++) {
...
#define CALL_IF(THREAD_M_BLOCKS/*4*/, THREAD_N_BLOCKS/*16*/, THREAD_K_BLOCKS/*4*/, GROUP_BLOCKS/*8*/) \
    ...
    ><<<blocks/*=92(sm)*/, THREADS, SHARED_MEM, stream/*0*/>>>( \
      ...
      locks, print_enable, user_specified_blockidx, user_specified_threadidx \
    ); \
  }
...
int marlin_cuda(
  ...
  int ret = 0;
  for (int i = 0; i < tot_m_blocks/*128*/; i += 8) {
    int thread_m_blocks = tot_m_blocks/*128*/ - i;
    prob_m = tot_m/*2048*/ - 16 * i;
    int par = 1;
    if (thread_m_blocks > 8) {
      // Note that parallel > 1 currently only works for inputs without any padding
      par = (16 * thread_m_blocks - pad) / (8*16);
      if (par > max_par/*16*/)
        par = max_par/*16*/;
      prob_m = 128 * par;
      i += 8 * (par - 1);
